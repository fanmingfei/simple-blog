...
更多信息可以参考，[为什么在JavaScript中使用静态类型](http://www.zcfy.cc/article/why-use-static-types-in-javascript-a-4-part-primer-on-static-typing-with-flow-3926.html)。
...
我们将这些文件或目录写在 `libs` 配置项中，这个配置对于我们使用 Flow 来说很重要。
...
如果对其做修改，更改传递给 `getStringLength` 的参数：
```js
// @flow
function getStringLength(str: string):number {
    const len: number = str.length;
